...
- 首先提出能力空间及其构建算法（纵轴）
- 映射贡献点就是提出两种新的数据评估维度（横轴）
- 梯度化：协同有效，尽量搞成在线算法(R&B)

- 得益于能力空间的优越性，我们提出两种基于能力空间的数据评估维度：
  - 能力空间目标相关度
  - 学习能力映射
...
- fuzz
  - 过一遍repo，proc
    - repo:
      - extract_api_from_head()
      - 复制docker中的目标目录到docker_shared/src
      - 分开找src和test头文件，使用cpp parser提取api，返回两个json
      ```
      ...
      ``` 
      - 其中的result_src返回给extract_src_test_api_call_graph
      - 对每个api生成一个egg，包含[函数名,
            文件路径,
            CodeQL 数据库路径,
            输出结果文件夹路径,
            LLM 工作目录路径]
      - 使用pool多线程并行运行handle_extract_api_call_graph_multiple_path，调用_extract_call_graph，运行                run_command([extract_shell_script, fn_name, fn_file_name, dbbase, outputfolder, str(pid)])其中extract_shell_script位于docker_shared/qlpacks/cpp_queries/extract_call_graph.sh，
      - 这个脚本会调用查询模板，相当于一个sql语句模板，去查询codeql数据库。查询模板用于构建函数的调用图，专门设计来查找从入口函数（如main或ENTRY_FNC）出发的所有直接函数调用
      - 查询结果为bqrs文件，转为csv
      - 也就是说对每一个api，查询其所有直接调用的api
      - 但是为啥这玩意生成的csv要么全一样要么没东西？
      - 好了修好了，改了生成模板
    - proc 
      - 合并所有api_list中的call_graph csv
      - 获取函数源码，src_api和初始化api_summary
    - fuzzing.py
      - 加载config，提取项目配置信息
      - config中包含项目头文件
      - 
